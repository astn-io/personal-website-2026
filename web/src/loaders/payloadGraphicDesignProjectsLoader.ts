...
type PayloadGraphicDesignProject = {
  id: string;
  slug: string;
  title: string;
  description: string;
...
function mapProject(project: PayloadGraphicDesignProject, baseUrl: string) {
...
export function payloadGraphicDesignProjectsLoader(options: LoaderOptions = {}): Loader {
  const baseUrl =
    options.url ??
  ...
  return {
    name: 'payload-graphic-design-projects-loader',
    load: async ({ store, parseData, logger }) => {
      ...
      try {
        while (hasNext) {
          const endpoint = new URL('/api/graphic-design-projects', baseUrl);
          ...
          if (!res.ok) {
            throw new Error(
              `Payload graphic design projects fetch failed: ${res.status} ${res.statusText} (${endpoint})`,
            );
          }

          const body = (await res.json()) as PayloadResponse<PayloadGraphicDesignProject>;
        ...
        logger.info(`Loaded ${total} graphic design projects from Payload at ${baseUrl}`);
      } catch (err) {
        logger.warn(
          `Payload graphic design projects loader failed (${baseUrl}): ${err instanceof Error ? err.message : String(err)}. Proceeding with an empty graphic design projects collection.`,
        );
      }
    },
  };
