...
type PayloadThreeDArtProject = {
  ...
  modelFile?: PayloadModelFile | string | null;
  downloadLabel?: string;
  previewModels?: PayloadModelFile[] | string[] | null;
...
function mapProject(project: PayloadThreeDArtProject, baseUrl: string) {
  ...
  const modelFile = isPopulated(project.modelFile) ? project.modelFile : undefined;
  const modelFileUrl = toAbsoluteUrl(baseUrl, modelFile?.url);

  const previewModels = (project.previewModels ?? [])
    .filter(isPopulated)
    .flatMap((file) => {
      const url = toAbsoluteUrl(baseUrl, file.url);
      if (!url) return [];
      return [
        {
          url,
  ...
  return {
    ...
    previewModels: previewModels.length > 0 ? previewModels : undefined,
...
export function payloadThreeDArtProjectsLoader(options: LoaderOptions = {}): Loader {
  const baseUrl =
    options.url ??
  ...
  return {
    name: 'payload-three-d-art-projects-loader',
    load: async ({ store, parseData, logger }) => {
      ...
      try {
        while (hasNext) {
          const endpoint = new URL('/api/three-d-art-projects', baseUrl);
          ...
          if (!res.ok) {
            throw new Error(
              `Payload 3D art projects fetch failed: ${res.status} ${res.statusText} (${endpoint})`,
            );
          }

          const body = (await res.json()) as PayloadResponse<PayloadThreeDArtProject>;
        ...
        logger.info(`Loaded ${total} 3D art projects from Payload at ${baseUrl}`);
      } catch (err) {
        logger.warn(
          `Payload 3D art projects loader failed (${baseUrl}): ${err instanceof Error ? err.message : String(err)}. Proceeding with an empty 3D art projects collection.`,
        );
      }
    },
  };
